...
fun loadData(liveDataToObserve: MutableLiveData<AppState>): MutableList<Categories> {
    ...
    Thread(Runnable {
        try {
            ...
            try {
                ...
                e.printStackTrace()
                handler.post { liveDataToObserve.postValue(AppState.Error(e)) }
            } finally {
            ...
            try {
                ...
                handler.post { liveDataToObserve.postValue(AppState.Success(categoriesData)) }
            } catch (e: Exception) {
                Log.e("", "Fail connection", e)
                e.printStackTrace()
                //Обработка ошибки
